 ...
 * Uses ZXing JS library for full 1D/2D barcode support including Code39 with hyphens.
 *
...
public class ScannerPageController {
    ...
    private String buildHtml(String token) {
        ...
        // ZXing: pure JS decoder, works on Safari iOS without native BarcodeDetector
        sb.append("<script src='https://unpkg.com/@zxing/library@0.21.3/umd/index.min.js'></script>");
        sb.append("<style>");
        sb.append("* { box-sizing:border-box; margin:0; padding:0; }");
        sb.append("body { font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; background:#0f172a; color:#e2e8f0; min-height:100vh; }");
        ...
        sb.append(".badge { background:#dbeafe; color:#1e3a8a; border-radius:20px; padding:4px 12px; font-size:12px; font-weight:600; margin-left:auto; }");
        sb.append(".container { padding:16px; max-width:480px; margin:0 auto; }");
        sb.append("#reader { border-radius:12px; overflow:hidden; border:2px solid #3b82f6; margin-bottom:16px; background:#000; }");
        ...
        sb.append(".card h2 { font-size:13px; color:#94a3b8; margin-bottom:12px; text-transform:uppercase; letter-spacing:0.05em; }");
        sb.append(".row { display:flex; gap:8px; margin-bottom:8px; }");
        sb.append("input { flex:1; padding:10px 14px; background:#0f172a; border:1px solid #334155; border-radius:8px; color:#e2e8f0; font-size:16px; }");
        sb.append("input:focus { outline:none; border-color:#3b82f6; }");
        sb.append(".qty { max-width:80px; }");
        sb.append("button { padding:10px 18px; border:none; border-radius:8px; font-size:15px; font-weight:600; cursor:pointer; }");
        sb.append(".btn-blue { background:#3b82f6; color:#fff; }");
        sb.append(".toast { position:fixed; bottom:24px; left:50%; transform:translateX(-50%); background:#10b981; color:#fff; padding:12px 24px; border-radius:24px; font-weight:700; font-size:15px; display:none; white-space:nowrap; box-shadow:0 4px 20px rgba(0,0,0,.4); }");
        ...
        sb.append("<table><thead><tr><th>SKU</th><th>Ten</th><th>Qty</th></tr></thead><tbody id='lineTable'></tbody></table>");
        sb.append("</div>");
        sb.append("</div>");
        ...
        // Init ZXing with all 1D+2D formats, TRY_HARDER for better decode
        ...
        sb.append("    ZXing.BarcodeFormat.QR_CODE,");
        sb.append("    ZXing.BarcodeFormat.CODE_128,");
        sb.append("    ZXing.BarcodeFormat.CODE_39,");
        sb.append("    ZXing.BarcodeFormat.CODE_93,");
        sb.append("    ZXing.BarcodeFormat.EAN_13,");
        sb.append("    ZXing.BarcodeFormat.EAN_8,");
        sb.append("    ZXing.BarcodeFormat.UPC_A,");
        sb.append("    ZXing.BarcodeFormat.UPC_E,");
        sb.append("    ZXing.BarcodeFormat.ITF,");
        sb.append("    ZXing.BarcodeFormat.CODABAR");
        sb.append("  ]);");
        ...
        sb.append("        submitScanWithBarcode(result.getText(),1);");
        sb.append("        setTimeout(function(){scanning=false;},1500);");
        sb.append("      }");
        sb.append("    });");
        sb.append("    setStatus('Camera san sang - huong vao barcode');");
        sb.append("  }).catch(function(err){setStatus('Camera khong kha dung: '+err);});");
        sb.append("});");

        sb.append("document.addEventListener('DOMContentLoaded',function(){");
        sb.append("  document.getElementById('barcodeInput').addEventListener('keydown',function(e){if(e.key==='Enter')submitScan();});");
        sb.append("});");

        sb.append("function submitScan(){");
        sb.append("  var b=document.getElementById('barcodeInput').value.trim();");
        ...
        sb.append("      catch(e){return {success:false,message:'Server loi ('+r.status+'): '+text.substring(0,120)};}");
        sb.append("    });");
        ...
        sb.append("      setStatus('Camera san sang');");
        sb.append("    }else{");
        ...
        sb.append("  var rows='';");
        sb.append("  for(var k in lineData){rows+='<tr><td>'+k+'</td><td>'+lineData[k].name+'</td><td>'+lineData[k].qty+'</td></tr>';}");
