...
public class NoSplitStrategy implements PutawayStrategy {
    ...
    public List<PutawayBinSuggestionDto> allocate(PutawaySuggestionContext ctx,
        ...
        for (ScoredBin sb : scoredBins) {
            ...
            dto.setSuggestedQuantity(qty);
            dto.setScore(sb.getScore());
            dto.setSplitUsed(false);
            dto.setMatchedRules(new ArrayList<>(sb.getMatchedRules()));
            dto.setExplanation("Allocated full quantity to bin " + bin.getLocation().getLocationCode());
