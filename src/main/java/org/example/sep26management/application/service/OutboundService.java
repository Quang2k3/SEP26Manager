    ...
    public ApiResponse<OutboundResponse> getOutboundDetail(Long documentId, String orderType) {
        if ("INTERNAL_TRANSFER".equals(orderType)) {
            TransferEntity transfer = transferRepository.findById(documentId)
                    .orElseThrow(() -> new org.example.sep26management.infrastructure.exception.ResourceNotFoundException("Transfer not found: " + documentId));
            ...
            SalesOrderEntity so = soRepository.findById(documentId)
                    .orElseThrow(() -> new org.example.sep26management.infrastructure.exception.ResourceNotFoundException("Sales order not found: " + documentId));
