...
public class BinService {
        ...
        // BR-LOC-22: inactive bins excluded from putaway (still shown in view with
        // status)
        ...
        public ApiResponse<PageResponse<BinOccupancyResponse>> viewBinOccupancy(
                ...
                Page<LocationEntity> binPage = locationRepository.searchLocations(
                        warehouseId, zoneId, LocationType.BIN, null, null, null,
                ...
                Set<Long> parentIds = binPage.getContent().stream()
                        .map(LocationEntity::getParentLocationId).filter(Objects::nonNull)
                ...
                // Build responses and apply occupancy status filter (in-memory after
                // calculation)
                List<BinOccupancyResponse> content = binPage.getContent().stream()
                        .map(bin -> {
                                BigDecimal occupied = occupiedMap.getOrDefault(bin.getLocationId(),
                                        BigDecimal.ZERO);
                                ...
                                LocationEntity rack = bin.getParentLocationId() != null
                                        ? parentMap.get(bin.getParentLocationId())
                                        : null;
                                ...
                                return BinOccupancyResponse.builder()
                                        ...
                                        .zoneCode(bin.getZoneId() != null
                                                ? zoneCodes.get(bin.getZoneId())
                                                : null)
                                        .parentLocationId(bin.getParentLocationId())
                                        .parentLocationCode(
                                                rack != null ? rack.getLocationCode() : null)
                        ...
                        .filter(r -> occupancyStatusFilter == null
                                || r.getOccupancyStatus() == occupancyStatusFilter)
                        .toList();
        ...
        @Transactional(readOnly = true)
        public ApiResponse<PageResponse<EmptyBinResponse>> searchEmptyBin(
                Long warehouseId,
                Long zoneId,
                ...
                Map<Long, String> parentCodes = locationRepository.findAllById(parentIds).stream()
                        .collect(Collectors.toMap(LocationEntity::getLocationId,
                ...
                List<EmptyBinResponse> result = allBins.stream()
                        .map(bin -> {
                                ...
                                return EmptyBinResponse.builder()
                                        ...
                                        .rackCode(bin.getParentLocationId() != null
                                                ? parentCodes.get(bin.getParentLocationId())
                                                : null)
                                        .maxWeightKg(bin.getMaxWeightKg())
                                        .maxVolumeM3(bin.getMaxVolumeM3())
                                        .occupiedQty(occupied)
                                        .availableWeightKg(availableWeight != null
                                                ? availableWeight.max(BigDecimal.ZERO)
                                                : null)
                                        .availableVolumeM3(availableVolume != null
                                                ? availableVolume.max(BigDecimal.ZERO)
                                                : null)
                                        .occupancyStatus(status)
                                        .isPickingFace(bin.getIsPickingFace())
                                        .build();
                        ...
                        .filter(r -> {
                                if (requiredWeightKg != null && r.getAvailableWeightKg() != null) {
                                        if (r.getAvailableWeightKg().compareTo(requiredWeightKg) < 0)
                                                return false;
                                }
                                if (requiredVolumeM3 != null && r.getAvailableVolumeM3() != null) {
                                        if (r.getAvailableVolumeM3().compareTo(requiredVolumeM3) < 0)
                                                return false;
                                }
                        ...
                        .sorted(Comparator.comparing(
                                r -> r.getAvailableWeightKg() != null ? r.getAvailableWeightKg()
                ...
                int fromIndex = Math.min(page * actualSize, totalElements);
                int toIndex = Math.min((page + 1) * actualSize, totalElements);
                List<EmptyBinResponse> paginatedContent = result.subList(fromIndex, toIndex);

                PageResponse<EmptyBinResponse> pageResponse = PageResponse.<EmptyBinResponse>builder()
                ...
                String message = paginatedContent.isEmpty()
        ...
        public ApiResponse<BinCapacityResponse> configureBinCapacity(
                ...
                if (currentOccupied == null)
                        currentOccupied = BigDecimal.ZERO;
                ...
                // Apply update
                if (request.getMaxWeightKg() != null)
                        bin.setMaxWeightKg(request.getMaxWeightKg());
                if (request.getMaxVolumeM3() != null)
                        bin.setMaxVolumeM3(request.getMaxVolumeM3());
                ...
                auditLogService.logAction(
                        ...
                        String.format("Bin %s capacity: weight=%s kg, volume=%s m³",
                                saved.getLocationCode(), saved.getMaxWeightKg(),
                                saved.getMaxVolumeM3()),
                        ipAddress, userAgent);
